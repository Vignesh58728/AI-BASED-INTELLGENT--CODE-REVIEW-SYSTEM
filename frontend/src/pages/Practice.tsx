...
import { Play, Send, HelpCircle, Zap, ChevronLeft, Wrench, ShieldAlert, Sparkles } from "lucide-react";
...
export function Practice() {
   ...
   const handleRun = async () => {
      setIsAnalyzing(true);
      setAiFeedback("⏳ Running your code...");
      try {
         // Try backend first
         const result = await submissionService.executeCode(code, language);
         if (result.status === "success") {
            setAiFeedback(`### ✅ Code Output\n\n\`\`\`\n${result.output || "(No output)"}\n\`\`\``);
         } else if (result.status === "timeout") {
            setAiFeedback(`### ⏱️ Timeout\n\n${result.error}`);
         } else {
            setAiFeedback(`### ❌ Error\n\n\`\`\`\n${result.error}\n\`\`\``);
         }
      } catch (e) {
         ...
         try {
            ...
            if (offlineResult.status === "success") {
               setAiFeedback(`### ✅ Code Output (Offline Mode)\n\n> [!NOTE]\n> Backend is unreachable. Running code in your browser.\n\n\`\`\`\n${offlineResult.output || "(No output)"}\n\`\`\``);
            } else {
               setAiFeedback(`### ❌ Offline Execution Error\n\n\`\`\`\n${offlineResult.error}\n\`\`\``);
            }
   ...
   const handleComplexity = async () => {
      setIsAnalyzing(true);
      try {
         const result = await submissionService.analyzeComplexity(code, "python");
         ...
         <div className="flex gap-4">
            ...
            <div className={`flex-1 grid grid-cols-1 lg:grid-cols-3 gap-4 transition-all duration-300 ${isChatOpen ? "w-[calc(100%-320px)]" : "w-full"}`}>
                              ...
                              if (/^(Input|Output|Explanation)\s*:/i.test(line.trim())) {
                                 ...
                                 return (
                                    <div key={i} className="font-mono text-sm rounded-lg px-4 py-1.5 border border-zinc-200 bg-zinc-50">
                                       <span className="font-bold text-zinc-900">{lbl}:</span>
                                       <span className="ml-1 text-green-700">{val}</span>
                                    </div>
                                 );
                              }
                              if (!line.trim()) return <br key={i} />;
                              return <p key={i} className="text-sm leading-relaxed text-zinc-700">{line}</p>;
                           })}
                        </div>
               ...
               <div className={`${isChatOpen ? "lg:col-span-3 xl:col-span-2" : "lg:col-span-2"} flex flex-col gap-4`}>
                        ...
                        <div className="absolute bottom-4 right-4 bg-white/95 border border-primary/20 rounded-xl p-3 shadow-lg z-10 max-w-xs animate-in fade-in slide-in-from-bottom-2">
                           ...
                           <div className="space-y-1.5">
                              {suggestions.map((s: string, i: number) => (
                                 <div
                                    key={i}
                                    className="text-[11px] text-zinc-600 bg-zinc-50 p-2 rounded-lg cursor-pointer hover:bg-primary/10 hover:text-primary transition-all border border-transparent hover:border-primary/20"
                  ...
                  {/* Feedback Panel */}
                  <div className="min-h-[200px] rounded-xl overflow-y-auto border border-zinc-200">
                     <FeedbackPanel feedback={aiFeedback} isLoading={isAnalyzing} />
                  </div>
               </div>
            </div>
         ...
         <div className="flex justify-between items-center w-full px-6 py-4 border-t border-zinc-200 bg-zinc-50/80 backdrop-blur-md shrink-0 rounded-b-2xl">
            <div className="flex gap-2">
               <Button
                  ...
                  className={isChatOpen ? "bg-primary text-white" : "h-9 text-xs text-zinc-600 hover:text-black"}
               >
                  <img src="/artificial-intelligence.png" alt="AI" className="mr-2 h-4 w-4" /> AI Tutor
               </Button>
               <Button variant="ghost" size="sm" onClick={handleExplain} disabled={isAnalyzing} className="h-9 text-xs text-zinc-600 hover:text-black">
                  <HelpCircle className="mr-2 h-4 w-4" /> Explain
               </Button>
               <Button variant="ghost" size="sm" onClick={handleComplexity} disabled={isAnalyzing} className="h-9 text-xs text-zinc-600 hover:text-black">
                  <img src="/career-path.png" alt="Complexity" className="mr-2 h-4 w-4" /> Complexity
               </Button>
               <Button variant="ghost" size="sm" onClick={handlePredictEdgeCases} disabled={isAnalyzing} className="h-9 text-xs text-zinc-600 hover:text-black">
            ...
            <div className="flex gap-3 items-center">
               ...
               <div className="border-l border-zinc-200 h-6 mx-1" />
               <Button variant="outline" size="sm" onClick={handleRun} disabled={isAnalyzing} className="h-9 text-xs gap-2 border-zinc-200 text-zinc-600 hover:bg-zinc-50">
