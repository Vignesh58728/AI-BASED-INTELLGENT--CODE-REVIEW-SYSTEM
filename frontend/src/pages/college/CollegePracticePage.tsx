...
export function CollegePracticePage() {
   ...
   // Inactivity timer for Proactive Analysis
   useEffect(() => {
   ...
   if (isLoading) {
      return (
         <div className="flex flex-col items-center justify-center min-h-screen bg-[#0a0a0a] text-black">
            <div className="animate-spin w-10 h-10 border-4 border-primary border-t-transparent rounded-full mb-4" />
            <p className="text-black font-medium">Loading problem details...</p>
         </div>
      );
   }
   ...
   if (!problem) {
      return (
         <div className="flex items-center justify-center min-h-screen bg-[#0a0a0a] text-black">
      ...
      <div className="h-screen flex flex-col bg-white text-black">
         ...
         <header className="h-14 border-b border-zinc-200 flex items-center px-6 justify-between bg-zinc-50">
            <div className="flex items-center gap-4">
               <Button variant="ghost" size="icon" onClick={() => navigate(-1)} className="text-black hover:text-black">
                  <ChevronLeft size={20} />
               </Button>
               <h2 className="font-bold flex items-center gap-2 text-black">
            ...
            <div className="flex items-center gap-3">
               <div className="text-xs text-black mr-4 hidden md:block">
               ...
               <Button
                  ...
                  className={isChatOpen ? "bg-primary text-black" : "border-zinc-200 text-black"}
               >
                  <Bot className="mr-2 h-4 w-4" /> Code Review
               </Button>
               ...
               <Button size="sm" onClick={handleSubmit} disabled={isAnalyzing} className="bg-primary text-black">
         ...
         <main className="flex-1 flex overflow-hidden">
            ...
            <div className={`w-1/3 border-r border-zinc-200 flex flex-col bg-zinc-50 transition-all duration-300 ${isChatOpen ? 'hidden lg:flex' : 'flex'}`}>
               <div className="flex-1 overflow-auto p-6 space-y-6">
                  <section>
                     <h3 className="text-sm font-bold text-primary uppercase tracking-widest mb-3 font-['Outfit']">Description</h3>
                           ...
                           if (part.startsWith('**Example')) {
                              ...
                              return (
                                 <div key={i} className="bg-white border border-zinc-200 rounded-xl p-4 my-4 shadow-sm">
                                    <p className="text-black font-bold mb-2 text-xs">{title.replace(/\*\*/g, '')}</p>
                                    <pre className="font-mono text-[11px] text-black bg-zinc-50 p-3 rounded-lg overflow-x-auto border border-zinc-100">
                                       {rest.join('\n').replace(/^\s*#+\s*/gm, '')}
                                    </pre>
                                 </div>
                              );
                           }
                           return <p key={i} className="whitespace-pre-wrap font-['Outfit']">{part.replace(/^\s*#+\s*/gm, '')}</p>;
                        })}
                     </div>
                  </section>
                  ...
                  <section className="bg-white rounded-xl border border-zinc-200 p-4 shadow-sm">
                     ...
                     <div className="text-xs text-black leading-relaxed whitespace-pre-wrap font-['Outfit']">
                        {aiFeedback?.replace(/^\s*#+\s*/gm, '') || "Start coding to get real-time AI feedback and logic analysis."}
            ...
            <div className="flex-1 flex flex-col relative overflow-hidden bg-white">
               ...
               {/* <span className="font-['Satisfy'] text-lg leading-none tracking-normal inline-flex items-center"><span className="text-zinc-900 font-bold">Mind</span><span className="text-zinc-400 ml-1">Arc</span></span> Suggestions */}
                  ...
                  <div className="absolute right-6 top-6 bg-white border border-primary/20 rounded-xl p-4 shadow-xl z-10 max-w-sm animate-in fade-in slide-in-from-right-4">
                     ...
                     <div className="space-y-2">
                        {suggestions.map((s, i) => (
                           <button
                              key={i}
                              className="w-full text-left text-xs text-black bg-zinc-50 py-2 px-3 rounded-lg hover:bg-primary/10 hover:text-primary transition-all border border-transparent hover:border-primary/20"
               ...
               <footer className="h-12 border-t border-zinc-200 bg-zinc-50 flex items-center px-4 justify-between">
                  <div className="flex gap-4">
                     <span className="text-[10px] font-mono text-black">{language}</span>
                     <span className="text-[10px] font-mono text-black">UTF-8</span>
                  </div>
                  <div className="flex gap-2">
                     <Button variant="ghost" size="sm" className="h-8 text-[10px] text-black hover:text-black" disabled={isAnalyzing} onClick={() => setCode("")}>
                        Clear Editor
                     </Button>
               ...
               {output && (
                  <div className="h-1/3 bg-zinc-50 border-t border-zinc-200 p-4 font-mono text-xs overflow-auto animate-in slide-in-from-bottom-4 shadow-inner">
                     <div className="flex justify-between items-center mb-2 border-b border-zinc-100 pb-2">
                        ...
                        <button className="text-zinc-600 hover:text-black" onClick={() => setOutput(null)}>Close</button>
                     </div>
                     <pre className="text-black leading-relaxed">{output}</pre>
                  </div>
               )}

               {/* Proactive Code Review Prompt */}
               {showProactivePrompt && (
                  <div className="absolute left-1/2 -translate-x-1/2 bottom-20 bg-primary/95 text-black px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-4 animate-in slide-in-from-bottom-10 backdrop-blur-md border border-white/20 z-100">
                     <div className="bg-white/20 p-2 rounded-xl">
                        <Bot size={24} />
                     </div>
                     <div>
                        <p className="text-sm font-bold">Stuck on this problem?</p>
                        <p className="text-[10px] text-black/80">I can teach you how to solve it step-by-step.</p>
                     </div>
